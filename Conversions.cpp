// Gerrick Keshawn Jones
// Dr. Sonya Dennis
// Computer Organization HCSC-260-01
// 10/13/2024
#include <iostream>
#include <string>
using namespace std;
...
int main()
{
	...
	bool isConverting = true; // Switch to end the program
	int choice; // Switch to selection conversion

	while (isConverting) 
	{
		// Initializing linked list with first node
		Node* head;
		Node* tail;
		Node* node = new Node();
		node->bit = 0;
		...
		// User will decide what values they would like to convert
		choice = DisplayOptions(); 

		// User input from previous function determines which function will be called
		switch (choice)
		{
		case 1:
		...
		case 2:
			...
			break;
		case 3:
			HexToBi(node, head, tail); // Hex to Binary
			break;
		case 4:
			BiToHex(PROMPT_TO_HEX, node, head, tail); // Binary to Hex 
			break;
		case 5:
			isConverting = quitProgram();
			break;
		}
	}
}
...
int DisplayOptions()
{
	...
	//Input validation: all non-digit inputs or inputs that are not 1-5 are invalid. 
	while(!(cin >> choice) || choice < 1 || choice > 5)
	{
		cin.clear();
		cin.ignore(1000, '\n');
		cout << "\nERROR: Integer is not within 1-5 OR input is not an integer"
			 << "\nChoice -> ";
	}
...
bool CheckHex(string hexNum)
{
	...
	if (hexNum.length() > 8)
	{
		cout << "\nERROR: More than 8 digits entered";
		return false;
	}
